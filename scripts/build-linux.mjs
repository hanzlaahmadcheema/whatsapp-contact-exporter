...
async function buildLinuxExecutable() {
  console.log('====================================================');
  console.log('   🐧 LINUX x64 PORTABLE EXECUTABLE BUILDER');
  console.log('====================================================\n');
  ...
  const targetBinaryPath = path.join(releaseDir, 'whatsapp-contact-exporter-linux-x64');
  ...
  // 5. Fetch/copy Node.js Linux x64 base binary
  console.log(`\n[Step 4/6] Resolving Node.js ${nodeVersion} Linux x64 base binary...`);
  fs.copyFileSync(process.execPath, targetBinaryPath);
  fs.chmodSync(targetBinaryPath, 0o755);
  console.log(`✓ Base binary prepared at: ${targetBinaryPath}`);

  // 6. Inject SEA Blob into Linux executable using postject
  console.log('\n[Step 5/6] Injecting SEA Blob into Linux binary...');
  ...
  console.log('\n====================================================');
  console.log('   🎉 LINUX EXECUTABLE BUILD SUCCESSFUL!');
  console.log('====================================================');
...
buildLinuxExecutable().catch((err) => {
  console.error('\n[Fatal Error] Linux executable build failed:', err);
  process.exit(1);
});
